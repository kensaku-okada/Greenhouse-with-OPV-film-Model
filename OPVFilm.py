...
def getOPVArea(OPVCoverageRatio):
    ...
    :return:  [m^2]
    '''
    return OPVCoverageRatio * constant.greenhouseRoofArea
...
def getMonthlyElectricityProductionFromDailyData (dailyJopvoutperArea, yearOfeachDay, monthOfeachDay):
    ...
    monthlyJopvoutperArea = np.zeros(numOfMonths)
    month = 0
    # insert the initial value
    monthlyJopvoutperArea[month] += dailyJopvoutperArea[0]
    for day in range (1, util.calcSimulationDaysInt()):
        if monthOfeachDay[day-1] <> monthOfeachDay[day]:
            month += 1
        monthlyJopvoutperArea[month] += dailyJopvoutperArea[day]

    return monthlyJopvoutperArea
